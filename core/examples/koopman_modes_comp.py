...
from matplotlib.pyplot import figure, grid, legend, plot, show, subplot, suptitle, title, ylim, xlabel, ylabel, xticks, yticks, imshow, colorbar
import os
...
from ..learning.utils import calc_koopman_modes, calc_reduced_mdl
import time
import dill
import control
...
noise_var = 0.                                         # Exploration noise to perturb controller

# Koopman eigenfunction parameters
eigenfunction_max_power = 3                             # Max power of variables in eigenfunction products
l2_diffeomorphism = 1e-3                                # l2 regularization strength
jacobian_penalty_diffeomorphism = 1e1#0.0                   # Estimator jacobian regularization strength
diff_n_epochs = 1000                                     # Number of epochs #TODO: Change
diff_train_frac = 0.9                                   # Fraction of data to be used for training
diff_n_hidden_layers = 2                                # Number of hidden layers
diff_layer_width = 20                                  # Number of units in each layer
diff_batch_size = 16                                    # Batch size
diff_learn_rate = 1e-2                               # Leaning rate
diff_learn_rate_decay = 0.975                            # Learning rate decay
diff_dropout_prob = 0.0                                # Dropout rate

# KEEDMD parameters
l1_pos_keedmd = 1e-3 #8.195946542380519e-07                   # l1 regularization strength for position states
l1_pos_ratio_keedmd = 1.0                               # l1-l2 ratio for position states
l1_vel_keedmd = 1e-3 #0.008926319071231337                    # l1 regularization strength for velocity states
l1_vel_ratio_keedmd = 1.0                               # l1-l2 ratio for velocity states
l1_eig_keedmd = 1e-3 #0.0010856707261463175                   # l1 regularization strength for eigenfunction states
...
l1_edmd_monomials = 1e-4 #0.015656845050848606                          # l1 regularization strength
...
l1_edmd_rbf = 1e-4 #0.015656845050848606                          # l1 regularization strength
l1_ratio_edmd_rbf = 1.0                                     # l1-l2 ratio
...
keedmd_model_full = Edmd(eigenfunction_basis, n, l1=l1_edmd_monomials, l1_ratio=l1_ratio_edmd_monomials)
...
A_red_keedmd, _, useful_cols_keedmd = calc_reduced_mdl(keedmd_model_full)
A_red_edmd_rbf, _, useful_cols_edmd_rbf = calc_reduced_mdl(edmd_model_rbf)
A_red_edmd_monominal, _, useful_cols_edmd_monomial = calc_reduced_mdl(edmd_model_monomial)
...
subplot(4,1,2)
bar(np.linspace(1,d.size, d.size, dtype=int)-1.5*width, np.flip(np.abs(d)), color='r', width=width, label='Analytic (3)')
bar(np.linspace(1,d_keedmd_f.size, d_keedmd_f.size, dtype=int)-0.5*width, np.flip(np.abs(d_keedmd_f)), width=width, label='KEEDMD (' + str(useful_cols_keedmd.size) +')')
bar(np.linspace(1,d_edmd_monomial.size, d_edmd_monomial.size, dtype=int)+0.5*width, np.flip(np.abs(d_edmd_monomial)), width=width, label='EDMD, monominal (' + str(useful_cols_edmd_monomial.size) +')')
bar(np.linspace(1,d_edmd_rbf.size, d_edmd_rbf.size, dtype=int)+1.5*width, np.flip(np.abs(d_edmd_rbf)), width=width, label='EDMD, rbf (' + str(useful_cols_edmd_rbf.size) +')')
...
title('Active Eigenvalue Magnitudes (# of active eigenpairs)')
xlabel('Eigenvalue #')
