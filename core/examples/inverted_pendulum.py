...
upper_bounds_mpc = array([np.Inf, np.Inf, np.Inf, np.Inf])  # State constraints, check they are higher than upper_bounds
lower_bounds_mpc = -upper_bounds_mpc  # State constraints
umax_mpc = 5  # check it is higher than the control to generate the trajectories
horizon_mpc = 0.4 # [s]
...
q_d_mpc = mpc_controller.parse_result()
output_pred = CartPoleTrajectory(system_true, q_d_mpc, t_pred_mpc)

# Nominal model MPC:
print(' - Simulating nominal model')
...
mse_mpc_nom = sum(sum((xs_nom_mpc-q_d_mpc)**2))/xs_nom_mpc.size
...
Q_d = Q.todense()
R_d = R.todense()
cost_nom = sum(np.diag(np.dot(np.dot((xs_nom_mpc-q_d_mpc).T,Q_d), xs_nom_mpc-q_d_mpc))) + sum(np.diag(np.dot(np.dot(us_nom_mpc.T,R_d),us_nom_mpc)))
...
data_list = [t_pred_mpc, q_d_mpc, xs_nom_mpc, xs_edmd_MPC, xs_keedmd_MPC, us_nom_mpc, us_edmd_MPC, us_keedmd_MPC, mse_mpc_nom, mse_mpc_edmd, mse_mpc_keedmd, E_nom, E_edmd, E_keedmd, cost_nom, cost_edmd, cost_keedmd]
...
for ii in range(n):
    ...
    plot(t_pred, xs_nom_mpc[ii, :], linewidth=2, label='Linearized dynamics with MPC', color='tab:green')
...
legend(fontsize=10, loc='best')
subplot(n + 1, 1, n + 1)
plot(t_pred[:-1], us_nom_mpc[0, :], linewidth=2, label='Linearized dynamics with MPC', color='tab:green')
